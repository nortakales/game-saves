...
LITTLE_ENDIAN=`printf "%08x\n" $SAV_SIZE | tac -rs .. | tr -d '\n' | sed -e "s/\(..\)/\\\\\\\\x\1/g"`
echo "Size as LE uint32: $LITTLE_ENDIAN"

WRITTEN_SIZE=$LITTLE_ENDIAN # Set to sav size in Drastic
PADDED_SIZE=$LITTLE_ENDIAN # Set to sav size in Drastic
...
SIZE_PARAMETER=$LITTLE_ENDIAN # Set to sav size in Drastic
